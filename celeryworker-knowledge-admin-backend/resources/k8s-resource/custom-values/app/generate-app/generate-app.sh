#!/bin/bash
# celeryworker-knowledge-admin-backend Deployment YAML 生成脚本
...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> celeryworker-knowledge-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
OUTPUT_DIR="$SCRIPT_DIR"

MAIN_DEPLOY_CONFIG="$PROJECT_ROOT/deploy-celeryworker-knowledge-admin-backend/app/deploy-app/deploy-celeryworker-knowledge-admin-backend.conf"
if [ -f "$MAIN_DEPLOY_CONFIG" ]; then
    _temp_namespace=$(source "$MAIN_DEPLOY_CONFIG" 2>/dev/null && echo "${CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_NAMESPACE:-}")
...
export CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE_REGISTRY="${CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE_REGISTRY:-}"
export CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE_PROJECT="${CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE_PROJECT:-}"
export CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE="${CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE:-}"
export CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_TAG="${CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_TAG:-}"
...
export CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME="${CELERYWORKER_KNOWLEDGE_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME:-}"
...
main() {
    log_info "开始生成 celeryworker-knowledge-admin-backend YAML 文件..."
