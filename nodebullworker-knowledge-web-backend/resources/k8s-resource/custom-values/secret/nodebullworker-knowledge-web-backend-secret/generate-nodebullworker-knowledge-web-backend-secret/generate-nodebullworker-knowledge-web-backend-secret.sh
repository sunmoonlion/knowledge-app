#!/bin/bash
# nodebullworker-knowledge-web-backend Node Bull Worker Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-nodebullworker-knowledge-web-backend-secret.conf"
...
if [ "${ENABLED:-true}" != "true" ]; then
    log_info "跳过资源生成: secret (已禁用)"; exit 0
fi
...
main() {
    log_info "开始生成 nodebullworker-knowledge-web-backend Node Bull Worker Secret YAML 文件..."
