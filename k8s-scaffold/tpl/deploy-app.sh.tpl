...
# __APP_NAME__ 部署脚本
# 用法: ./deploy-__APP_NAME__.sh <deploy|uninstall|status> [project_id] [namespace] [environment]

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 从 app/deploy-app/ 向上 2 级到达 deploy-__APP_NAME__/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 向上 1 级到达应用根目录
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

__APP_NAME_UPPER___SCRIPT_DIR="$SCRIPT_DIR"
...
SCRIPT_DIR="$__APP_NAME_UPPER___SCRIPT_DIR"
...
__APP_NAME_UPPER___CONFIG_FILE="$SCRIPT_DIR/deploy-__APP_NAME__.conf"
if [[ -f "$__APP_NAME_UPPER___CONFIG_FILE" ]]; then
    source "$__APP_NAME_UPPER___CONFIG_FILE"
    ...
    log_info "已加载 __APP_NAME__ 配置文件: $__APP_NAME_UPPER___CONFIG_FILE"
else
    log_warn "未找到 __APP_NAME__ 配置文件: $__APP_NAME_UPPER___CONFIG_FILE，使用默认配置"
fi

DEFAULT_PROJECT_ID="${__APP_NAME_UPPER___PROJECT_ID:-}"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"__APP_NAME__-pvc-generated.yaml" ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/__APP_NAME__-pvc/generate-__APP_NAME__-pvc/generate-__APP_NAME__-pvc.sh"
    else
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    fi

    if [ -f "$generate_script" ]; then
        export NAMESPACE="${__APP_NAME_UPPER___NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${__APP_NAME_UPPER___PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_success "✅ __APP_NAME__ 子组件部署完成"
    return 0
}

deploy_app() {
    log_info "开始部署 __APP_NAME__..."
    ...
    export __APP_NAME_UPPER___IMAGE_PULL_SECRET_NAME="${__APP_NAME_UPPER___IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"
    ...
    auto_generate_yaml "$__APP_NAME_UPPER___YAML" "$K8S_RESOURCE_DIR" || return 1

    log_info "生成并部署 PVC..."
    auto_generate_yaml "$__APP_NAME_UPPER___PVC_YAML" || return 1
    kubectl apply -f "$__APP_NAME_UPPER___PVC_YAML" -n "$NAMESPACE" \
        && log_success "PVC 部署完成" \
        || { log_error "PVC 部署失败"; return 1; }

    kubectl apply -f "$__APP_NAME_UPPER___YAML" -n "$NAMESPACE"

    if [ $? -eq 0 ]; then
        log_success "__APP_NAME__ 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=__APP_NAME__"
        echo "  kubectl logs -n $NAMESPACE -l app=__APP_NAME__ -f"
    else
        log_error "__APP_NAME__ 部署失败"
        exit 1
    fi
}

uninstall_app() {
    log_info "开始卸载 __APP_NAME__..."

    log_info "🚀 阶段1：卸载核心服务..."
    if [ ! -f "$__APP_NAME_UPPER___YAML" ]; then
        log_warn "生成的 YAML 不存在，直接删除资源"
        kubectl delete deployment __APP_NAME__ -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service __APP_NAME__ -n "$NAMESPACE" --ignore-not-found=true
    else
        kubectl delete -f "$__APP_NAME_UPPER___YAML" -n "$NAMESPACE" --ignore-not-found=true
    fi
    ...
    log_success "__APP_NAME__ 卸载完成！"
}

show_status() {
    log_info "__APP_NAME__ 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=__APP_NAME__ 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=__APP_NAME__ 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=__APP_NAME__ 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=__APP_NAME__ 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=__APP_NAME__ 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
    kubectl get pvc -n "$NAMESPACE" -l app=__APP_NAME__ 2>/dev/null || echo "  无 PVC"
}
